## plot_spectrogram_images_per_deployment ##


# input -------------------------------------------------------------------

# acoustic data directory
data_dir = 'data/raw/2017/noaa/data/2017-07-24_DEP19/'

# deployment id
dep_id = '19'

# processed data directory (with selection tables)
proc_dir = 'data/raw/2017/noaa/processed/'

# deployment basename
bname = 'DEP'

# ouput directory
out_dir = 'figures/selection_table_images/2017_noaa_raven_images/'
...
sfile = paste0(proc_dir, bname, dep_id, '_selections.txt')
...
# excluding the long selections
dur_sel = sel$tmax - sel$tmin
max_dur = 60 
ind = which(dur_sel >= max_dur)
sel = sel[-ind, ]
#sel$call_type[ind] to see which call types are being excluded
message(length(ind), ' selections removed because exceeds ', max_dur, ' seconds.')
...
# message for which deployment starting on
message(dep_id, ' started!')
...
for(kk in 1:nrow(sel)){
  ...
  # add text
  mtext(text = paste0(dep_id, ': ', sel$time[kk]), side = 3, line = -1, adj = 0)
  
  # add bottom text
  mtext(paste0('Notes: ', sel$notes[kk]), side = 1, line = 3, adj = 0)
  mtext(paste0('Score: ', sel$score[kk]), side = 1, line = 4, adj = 0)
  
  # save plot
  dev.off()
  
  message('    Selection ', sel$selection[kk], ' complete!')
