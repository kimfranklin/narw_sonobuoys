...
```{r, echo=FALSE}
df3=df%>%
  mutate(total_behv=foraging+social+other_bhv)
total_stats = summary(df3$total_bev)
total_stats
total_stdev = sd(df3$total_bhv)
total_stdev
```
...
#number of whales by behaviour total
#feed
feed_stats = summary(df$foraging)
feed_stats
feed_stdev = sd(df$foraging)
feed_stdev
```
...
```{r, echo=FALSE}
#other behaviour
other_behv_stats = summary(df$other_bhv)
other_behv_stats
other_behv_stdev = sd(df$other_bhv)
other_behv_stdev
```
